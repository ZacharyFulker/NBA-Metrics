...
#nba.df2$Metric1 <- (nba.df2$Metric)^2
...
nba.df2_15v <- subset(nba.df2, select = c(Year, X3P, X3PA, X2P, X2PA, FT, FTA, ORB, DRB, AST, STL, BLK, TOV, PF, ORBp, DRBp, ASTp, STLp, BLKp, TOVp, USGp, VORP, Metric))
...
nsim <- 25
imp.mat <- matrix(0,nrow=nsim,ncol=dim(nba.df2_15v)[2]-1)
for (i in 1:nsim) {
  
  # Build the random Forest
  rf <- randomForest(VORP~.,data=nba.df2_15v,ntree=500,importance=TRUE)
...
rank.feat <- matrix(0,nrow=nsim,ncol=dim(nba.df2_15v)[2]-1)
for (i in 1:nsim) {
  rnk <- order(-imp.mat[i,])
...
nba.df2_15v1 <- subset(nba.df2_15, select = c(Year, X3P, X3PA, X2P, X2PA, FT, FTA, ORB, DRB, AST, STL, BLK, TOV, PF, TSp, ORBp, DRBp, ASTp, STLp, BLKp, TOVp, USGp, MP, VORP))

#Use 10-fold validation for standard RF and LM:
#Create 10 equally size folds
folds <- cut(seq(1,nrow(nba.df2_15v1)),breaks=10,labels=FALSE)
#Perform 10 fold cross validation
for(i in 1:10){
  testIndexes <- which(folds==i,arr.ind=TRUE)
  df.test <- nba.df2_15v1[testIndexes, ]
  df.train <- nba.df2_15v1[-testIndexes, ]
  ...
  nba.lm <- lm(VORP~.,data=nba.df2_15v1)
  lm.preds <- predict(nba.lm, nba.df2_15v1)
  lm.diff <- nba.df2_15v1$VORP - lm.preds
  nba.df.diff <- nba.df2_15v1
  ...
  nba.true <- nba.df2_15v1[testIndexes,]
  # Build the RF:
